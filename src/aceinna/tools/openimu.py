...
class OpenIMU(object):
    ...
    def getdata(self, datatype):
        ...
        if datatype == ('S1'):
            pi = 3.14159265359
            xaccel = struct.unpack('H', bytes(readback[0:2]))[0]*20/(2**16)
            yaccel = struct.unpack('H', bytes(readback[2:4]))[0]*20/(2**16)
            zaccel = struct.unpack('H', bytes(readback[4:6]))[0]*20/(2**16)
